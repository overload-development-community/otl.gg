            ...
            <div id="records">
                ...
                ${[2, 3, 4].map((teamSize) => /* html */`
                    ...
                    ${records.teamKda.filter((r) => r.teamSize === teamSize).map((r) => /* html */`
                        <div class="tag"><div class="diamond${(team = teams.getTeam(r.teamId, r.teamName, r.tag)).role && team.role.color ? "" : "-empty"}" ${team.role && team.role.color ? `style="background-color: ${team.role.hexColor};"` : ""}></div> <a href="/team/${team.tag}">${team.tag}</a></div>
                        ...
                        <div><script>document.write(formatDate(new Date("${r.matchTime}")));</script></div>
                        <div>${r.map}</div>
                    `).join("")}
                `).join("")}
