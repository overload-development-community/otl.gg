...
class RecordsView {
     ...
     */
    static get(data) {
        const {seasonList, records, season, postseason, gameType, gameTypeName, recordType, teamId, teams, teamList} = data;
        let team;

        return /* html */`
            <div id="options">
                <span class="grey">Season:</span> ${seasonList.map((seasonNumber, index) => /* html */`
                    ${!isNaN(season) && season !== seasonNumber || isNaN(season) && index + 1 !== seasonList.length ? /* html */`<a href="/records?gameType=${gameType}&recordType=${recordType}${teamId ? `&teamId=${teamId}` : ""}&season=${seasonNumber}${postseason ? "&postseason=yes" : ""}">${seasonNumber}</a>` : seasonNumber}
                `).join(" | ")} | ${season === 0 ? "All Time" : /* html */`<a href="/records?gameType=${gameType}&recordType=${recordType}${teamId ? `&teamId=${teamId}` : ""}&season=0${postseason ? "&postseason=yes" : ""}">All Time</a>`}<br />
                <span class="grey">Postseason:</span> ${postseason ? "Yes" : /* html */`<a href="/records?gameType=${gameType}&recordType=${recordType}${teamId ? `&teamId=${teamId}` : ""}&postseason=yes${isNaN(season) ? "" : `&season=${season}`}">Yes</a>`} | ${postseason ? /* html */`<a href="/records?gameType=${gameType}&recordType=${recordType}${teamId ? `&teamId=${teamId}` : ""}${isNaN(season) ? "" : `&season=${season}`}">No</a>` : "No"}<br />
                <span class="grey">Game Type:</span> ${gameType === "TA" ? "Team Anarchy" : /* html */`<a href="/records?gameType=TA&recordType=${recordType}${teamId ? `&teamId=${teamId}` : ""}${isNaN(season) ? "" : `&season=${season}${postseason ? "&postseason=yes" : ""}`}">Team Anarchy</a>`} | ${gameType === "CTF" ? "Capture the Flag" : /* html */`<a href="/records?gameType=CTF&recordType=${recordType}${teamId ? `&teamId=${teamId}` : ""}${isNaN(season) ? "" : `&season=${season}${postseason ? "&postseason=yes" : ""}`}">Capture the Flag</a>`}<br />
                <span class="grey">Records:</span> ${recordType === "team" ? "Team" : /* html */`<a href="/records?gameType=${gameType}&recordType=team${teamId ? `&teamId=${teamId}` : ""}${isNaN(season) ? "" : `&season=${season}${postseason ? "&postseason=yes" : ""}`}">Team</a>`} | ${recordType === "player" ? "Player" : /* html */`<a href="/records?gameType=${gameType}&recordType=player${teamId ? `&teamId=${teamId}` : ""}${isNaN(season) ? "" : `&season=${season}${postseason ? "&postseason=yes" : ""}`}">Player</a>`}<br />
                <span class="grey">Team:</span> ${teamList.sort((a, b) => a.name.localeCompare(b.name)).map((teamData) => /* html */`
                    &nbsp;<div class="diamond${teamData.color ? "" : "-empty"}" ${teamData && teamData.color ? `style="background-color: ${teamData.color};"` : ""}></div>
                    ${!teamId || teamId !== teamData.teamId ? /* html */`<a href="/records?gameType=${gameType}&recordType=${recordType}${isNaN(season) ? "" : `&season=${season}${postseason ? "&postseason=yes" : ""}`}&teamId=${teamData.teamId}">${teamData.tag}</a>` : teamData.tag}
                        ...
                        ${records.teamDeaths.length === 0 ? "" : /* html */`
                            ...
                            ${records.teamDeaths.map((r) => r.teamSize).filter((r, index, a) => a.indexOf(r) === index).sort((a, b) => a - b).map((teamSize) => /* html */`
                                ...
                                ${records.teamDeaths.filter((r) => r.teamSize === teamSize).map((r) => /* html */`
                                    <div class="tag team-tag"><div class="diamond${(team = teams.getTeam(r.teamId, r.teamName, r.tag)).role && team.role.color ? "" : "-empty"}" ${team.role && team.role.color ? `style="background-color: ${team.role.hexColor};"` : ""}></div> <a href="/team/${team.tag}">${team.tag}</a></div>
                                    <div class="team team-name"><a href="/team/${team.tag}">${team.name}</a></div>
                                    <div><span class="numeric">${r.record}</span> &nbsp;Death${r.record === 1 ? "" : "s"}</div>
                                    <div class="tag"><div class="diamond${(team = teams.getTeam(r.opponentTeamId, r.opponentTeamName, r.opponentTag)).role && team.role.color ? "" : "-empty"}" ${team.role && team.role.color ? `style="background-color: ${team.role.hexColor};"` : ""}></div> <a href="/team/${team.tag}">${team.tag}</a></div>
                                    <div class="team-name"><a href="/team/${team.tag}">${team.name}</a></div>
                                    <div class="date"><a href="/match/${r.challengeId}/${r.tag}/${r.opponentTag}"><script>document.write(Common.formatDate(new Date("${r.matchTime}")));</script></a></div>
                                    <div class="map">${r.map}${r.overtimePeriods > 0 ? `, ${r.overtimePeriods > 1 ? r.overtimePeriods : ""}OT` : ""}</div>
