/**
 * @typedef {import("../../types/viewTypes").TeamCTFMapViewParameters} ViewTypes.TeamCTFMapViewParameters
 */
...
class TeamCTFMapView {
     ...
     * @param {ViewTypes.TeamCTFMapViewParameters} data The team data.
        ...
        return /* html */`
            <div id="team">
                <div id="teamname">
                    ...
                    <div class="map">Game Performance in ${map} for Capture the Flag</div>
                </div>
            </div>
            <div class="options">
                <span class="grey">Season:</span> ${seasonList.map((seasonNumber, index) => /* html */`
                    ${!isNaN(season) && season !== seasonNumber || isNaN(season) && index + 1 !== seasonList.length ? /* html */`<a href="/team/${encodeURI(pageTeam.tag)}/map/ctf-${map}?season=${seasonNumber}${postseason ? "&postseason=yes" : ""}">${seasonNumber}</a>` : seasonNumber}
                `).join(" | ")} | ${season === 0 ? "All Time" : /* html */`<a href="/team/${encodeURI(pageTeam.tag)}/map/ctf-${map}?season=0${postseason ? "&postseason=yes" : ""}">All Time</a>`}<br />
                <span class="grey">Postseason:</span> ${postseason ? "Yes" : /* html */`<a href="/team/${encodeURI(pageTeam.tag)}/map/ctf-${map}?postseason=yes${isNaN(season) ? "" : `&season=${season}`}">Yes</a>`} | ${postseason ? /* html */`<a href="/team/${encodeURI(pageTeam.tag)}/map/ctf-${map}${isNaN(season) ? "" : `?season=${season}`}">No</a>` : "No"}
            </div>
                ...
                ${teamData.stats.length === 0 ? "" : /* html */ `
                    ...
                    <div class="stats stats-ctf">
                        ...
                        ${teamData.stats.map((s) => /* html */`
                            <div class="player"><a href="/player/${s.playerId}/${encodeURIComponent(TeamCTFMapView.Common.normalizeName(s.name, team.tag))}">${TeamCTFMapView.Common.htmlEncode(TeamCTFMapView.Common.normalizeName(s.name, team.tag))}</a></div>
                        ...
                        <div id="matches">
                            ...
                            ${teamData.gameLog.map((m) => /* html */`
                                ...
                                <div class="player">${m.playerId ? /* html */`
                                    <a href="/player/${m.playerId}/${encodeURIComponent(TeamCTFMapView.Common.normalizeName(m.name, team.tag))}">${TeamCTFMapView.Common.htmlEncode(TeamCTFMapView.Common.normalizeName(m.name, team.tag))}</a>
                                ` : ""}</div>
...
// @ts-ignore
TeamCTFMapView.Common = typeof Common === "undefined" ? require("../../web/includes/common") : Common; // eslint-disable-line no-undef

if (typeof module !== "undefined") {
    module.exports = TeamCTFMapView; // eslint-disable-line no-undef
}
