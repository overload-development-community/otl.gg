...
class Commands {
    ...
    async version(member, channel, message) {
        ...
        await Discord.queue(`We are The Fourth Sovereign, we are trillions.  By roncli, Version ${pjson.version}.  Project is open source, visit https://github.com/overload-development-community/otl.gg.`, channel);
        return true;
    }
    ...
    async suggestteamsize(member, channel, message) {
        ...
        if (["2", "3", "4", "5", "6", "7", "8", "2v2", "3v3", "4v4", "5v5", "6v6", "7v7", "8v8"].indexOf(message.toLowerCase()) === -1) {
            await Discord.queue(`Sorry, ${member}, but that is an invalid team size.  To suggest a team size, use the \`!suggestteamsize <size>\` command, for example, \`!suggestteamsize 3\`.`, channel);
    ...
    async start(user, channel, message) {
        ...
        Azure.setup(settings.servers[message], message, channel);
